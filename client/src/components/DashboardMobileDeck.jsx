...
export default function DashboardMobileDeck({
  ...
  useEffect(() => {
    if (!phone || !railRef.current) return;
    const btn = railRef.current.querySelector(`[data-tab-id="${panelTab}"]`);
  ...
  return (
    <div className={`dash-mobile-deck dash-mobile-deck--${variant}${burst ? ' dash-mobile-deck--burst' : ''}`}>
      ...
      <div className="dash-mobile-deck__scan" aria-hidden />
      ...
      {user && (
        <button
          type="button"
          className={`dash-mobile-deck__flip${flipped ? ' dash-mobile-deck__flip--back' : ''}`}
        ...
        >
          <div className="dash-mobile-deck__flip-inner">
            ...
            <div className="dash-mobile-deck__flip-face dash-mobile-deck__flip-face--back">
              ...
              <ChevronRight className="w-4 h-4 opacity-40 shrink-0 rotate-180" aria-hidden />
            </div>
          </div>
        </button>
      )}
      ...
      <div className="dash-mobile-deck__rail-wrap">
        <p className="dash-mobile-deck__rail-label">Swipe sections</p>
        <div className="dash-mobile-deck__rail" ref={railRef} role="tablist" aria-label="Dashboard sections">
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`dash-mobile-deck__chip${isActive ? ' dash-mobile-deck__chip--active' : ''}`}
