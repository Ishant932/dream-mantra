...
function ResourceLinkButton({ href, label, icon: Icon = Link2 }) {
  if (!href) return null;
  return (
    <a href={href} target="_blank" rel="noreferrer" className="training-resource-btn">
    ...
    <div className="training-resource-card">
      <div className="training-resource-card__head">
        ...
        <div className="min-w-0">
          <p className="font-semibold text-theme-primary text-sm">From Dream Mantra team</p>
          <p className="text-xs dash-card-meta">
            {new Date(msg.created_at).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}
          </p>
        </div>
      </div>
      {msg.body && <p className="text-sm dash-card-meta mt-3 whitespace-pre-wrap leading-relaxed">{msg.body}</p>}
      {links.length > 0 && (
        <div className="mt-3 flex flex-col gap-2">
          {links.map((url) => <ResourceLinkButton key={url} href={url} label="Open resource link" />)}
        </div>
      )}
      {attachments.length > 0 && (
        <div className="mt-3 flex flex-col gap-2">
          {attachments.map((att, i) => (
            <ResourceLinkButton key={i} href={att.url} label={att.name || 'Download document'} icon={FileText} />
          ))}
        </div>
      )}
    </div>
...
export default function TrainingResourcesPanel({ token, resources: sharedResources = [] }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMessages((data.messages || []).filter((m) => m.sender_role === 'admin'));
      setError('');
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12 text-sm dash-card-meta gap-2">
        <Loader2 className="w-5 h-5 animate-spin text-amber-600" /> Loading resources…
      </div>
    );
  }
  ...
  if (!messages.length && !sharedResources.length) {
    return (
      <div className="rounded-2xl border border-dashed border-amber-300/70 bg-amber-50/50 p-8 text-center">
        ...
        <p className="text-sm dash-card-meta mt-2 max-w-md mx-auto">
          When your counsellor or admin shares links, notes, or documents, they will appear here.
        </p>
      </div>
    );
  }
    ...
    <div className="grid gap-4 md:grid-cols-2">
      {sharedResources.map((r) => (
        <div key={r.id} className="training-resource-card">
          ...
          <div className="mt-3">
            <ResourceLinkButton href={r.url} label={r.title || 'Open resource'} />
          </div>
        </div>
      ))}
      {messages.map((msg) => <ResourceCard key={msg.id} msg={msg} />)}
    </div>
  );
}
